...
my $infile = '/data/R02/huangtt39/ATAC-RNAseq/snp/129/129P2_OlaHsd.mgp.v5.snps.dbSNP142.vcf.gz'; 
...
warn "SNP position summary for strain 129S1\n\n";

warn "$homozygous\tSNP were homozygous. Of these:\n";
warn "$hcg_count\tSNP were homozygous and passed high confidence filters, and were included into 129S1 genome\n";

warn "\nNot included into 129S1 genome:\n";
...
open (OUT,'>','all_129_SNPs.txt') or die $!;

foreach my $location (keys %all_SNPs){
  print OUT "$all_SNPs{$location}\n";
}
open (OUT,'>','origin_129_SNPs.txt') or die $!;
