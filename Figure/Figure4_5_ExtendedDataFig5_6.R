...
ORgene_FC_p <-ggplot(data=OR_count,aes(x=log2((Paternal_specific+0.1)/(Maternal_specific+0.1))))+
      ...
      coord_flip()
ggsave(filename = "/data/R02/huangtt39/F1_OSN/analysis/figure/Figure3/OR_cutoff.pdf",ORgene_cellnumber_p+plot_spacer()+ORgene_point_p+ORgene_FC_p+plot_layout(nrow=2,ncol=2,height=c(1,2,1,2),width=c(2,1,2,1)), device = "pdf", width =12, height = 9, units = "cm", dpi = 60,bg="white")
...
ggsave(filename =  "/data/R02/huangtt39/F1_OSN/analysis/figure/Figure3/OR_sample_distance_point.pdf",sample_distance_point_p+plot_layout(ncol=4,guides = 'collect'), device = "pdf", width =32, height = 8, units = "cm", dpi = 300,bg="white")
...
write.xlsx(OR_distance,file="/data/R02/huangtt39/F1_OSN/analysis/figure/Figure6/OR_distance_SNP.xlsx")
OR_distance<-read.xlsx("/data/R02/huangtt39/F1_OSN/analysis/figure/Figure6/OR_distance_SNP.xlsx")
...
ggsave(filename = "/data/R02/huangtt39/F1_OSN/analysis/figure/Figure6/OR_difference_ranking.pdf",OR_difference_ranking, device = "pdf", width=16, height = 8, units = "cm", dpi = 300,bg="white")
...
pdf("/data/R02/huangtt39/F1_OSN/analysis/figure/Figure6/OR_distance_heatmap.pdf",height=6.5,width=8.5)
...
pdf("/data/R02/huangtt39/F1_OSN/analysis/figure/Figure6/OR_distance_violin.pdf",height=2.5,width=3.5)
...
ggsave(filename = "/data/R02/huangtt39/F1_OSN/analysis/figure/Figure6/OR_high_distance_umap.pdf",OR_high_distance_umap1+OR_high_distance_umap2 & NoLegend() & theme(plot.title = element_text(hjust = 0.5,size=8),text=element_text(size=8),axis.text.y=element_text(size=8,color = "black"), axis.text.x=element_text(size=8,  color = "black")), device = "pdf", width =20, height = 10, units = "cm", dpi = 300)
...
saveWorkbook(wb, "/data/R02/huangtt39/F1_OSN/analysis/figure/Figure6/OR_differential_gene.xlsx", overwrite = TRUE)
...
ggsave(filename = "/data/R02/huangtt39/F1_OSN/analysis/figure/Figure6/OR_marker_Volcano_Plot.pdf",marker_p+plot_layout(ncol=4,guides="collect"), device = "pdf", width = 34, height = 8, units = "cm", dpi = 300,bg="white")
...
pdf("/data/R02/huangtt39/F1_OSN/analysis/figure/Figure6/OR_DEG_heat.pdf",height=4.5,width=8)
for(i in 1:3){
  OR_DEG_heat(i)
}
dev.off()
...
saveWorkbook(wb, "/data/R02/huangtt39/F1_OSN/analysis/figure/Figure6/OR_differential_peak.xlsx", overwrite = TRUE)
...
pdf("/data/R02/huangtt39/F1_OSN/analysis/figure/Figure6/OR_DEP_heat.pdf",height=4.5,width=6.5)
for(i in 1:3){
  OR_DEP_heat(i)
}
dev.off()
...
ggsave(filename = "/data/R02/huangtt39/F1_OSN/figure/Figure6/unsyn_snp_distance.pdf",SNP_VS_length_p+distance_VS_snp_p+plot_layout(width=c(1,1.5)), device = "pdf", width = 21, height = 8, units = "cm", dpi = 60,bg="white")
...
classII_OR_consense<-read.xlsx("/data/R02/huangtt39/F1_OSN/analysis/merge/OR_annalysis/conservation.xlsx",sheet = 1)
...
write.xlsx(classII_OR_consense,file="/data/R02/huangtt39/F1_OSN/analysis/figure/Figure6/GPCR/conservation2color.xlsx",sheet = 1)
...
distance_conversation_p<-ggplot(classII_OR_consense%>%subset(distance%in%c("high","low")), aes(x = difference, y = Max.ratio, color = distance)) +
  ...
  theme_classic() +  # 使用简洁主题
  scale_color_manual(values=c("#0E8A8F","#F44040"))
ggsave(filename = "/data/R02/huangtt39/F1_OSN/analysis/figure/Figure6/GPCR/distance_conversation.pdf",distance_conversation_p, device = "pdf", width = 10, height = 5, units = "cm", dpi = 300)
