...
Epha7<-data.frame(allele_exp_type=c("P","P","P","M","M","M","B","B","B","P","M","B","P","M","B"),rep=c("rep1","rep2","rep3","rep1","rep2","rep3","rep1","rep2","rep3","rep1","rep1","rep1","rep1","rep1","rep1"),
                ratio=c(29.5,32.6,41.7,39.3,34.9,27.1,31.1,32.6,31.2,11.3,7.6,81,22.6,17.1,60.3),
...
Epha7_p<-ggplot(Epha7,aes(x=method,y=ratio,fill=allele_exp_type,color=allele_exp_type))+
    ...
    scale_color_manual(values=c("#dfe2d6","#c68989","#94b2c2"))+
...
ggsave(filename = "/data/R02/huangtt39/ATAC-RNAseq/analysis/Fish/experimental/Epha7_Fishresult.pdf",Epha7_p, device = "pdf", width = 11, height = 5, units = "cm", dpi = 60,bg="white")
...
Peg3<-data.frame(allele_exp_type=c("P","P","P","M","M","M","B","B","B","P","M","B","P","M","B"),rep=c("rep1","rep2","rep3","rep1","rep2","rep3","rep1","rep2","rep3","rep1","rep1","rep1","rep1","rep1","rep1"),
                ratio=c(90,92,88.2,10,0,0,0,8,11.8,93.3,0,6.7,100,0,0),
                method=c(rep("RNA_Fish",9),rep("Multiomics-seq",3),rep("Single-cell-RNA-seq",3)))
...
ggsave(filename = "/data/R02/huangtt39/ATAC-RNAseq/analysis/Fish/experimental/Peg3_Fishresult.pdf",Peg3_p, device = "pdf", width = 11, height = 5, units = "cm", dpi = 60,bg="white")
...
Adcy3<-data.frame(allele_exp_type=c("P","P","P","M","M","M","B","B","B","P","M","B","P","M","B"),rep=c("rep1","rep2","rep3","rep1","rep2","rep3","rep1","rep2","rep3","rep1","rep1","rep1","rep1","rep1","rep1"),
                ratio=c(0,0,0,0,0,2,100,100,98,4.1,6.6,89.3,9.4,14.5,76.1),
                method=c(rep("RNA_Fish",9),rep("Multiomics-seq",3),rep("Single-cell-RNA-seq",3)))
...
ggsave(filename = "/data/R02/huangtt39/ATAC-RNAseq/analysis/Fish/experimental/Adcy3.pdf",Adcy3_p, device = "pdf", width = 11, height = 5, units = "cm", dpi = 60,bg="white")
...
Epha7_OR_sample30<-VlnPlot(
  ...
  features = c("Epha7"),
  group.by =  "expressed_OR",
  pt.size =0,slot = 'scale.data')&geom_boxplot()&NoLegend()&scale_fill_manual(values=rep("#507b8a",30))
ggsave(filename = "/data/R02/huangtt39/ATAC-RNAseq/analysis/figure/Figure3/Epha7_OR_sample30.pdf",Epha7_OR_sample30, device = "pdf", width = 40, height = 10, units = "cm", dpi = 300)
...
OR_group_up<-OR_group_up%>%mutate(avg_exp_type=case_when(avg_exp>=0&avg_exp<~"low",avg_exp>=6~"high"))
...
# merge
OR_group_up<-merge(OR_group_up,OR_OB%>%select(-cell),by="expressed_OR")
...
# Figure 4E
OR_group_up$scale_x<-(OR_group_up$x - min(OR_group_up$x)) / (max(OR_group_up$x) - min(OR_group_up$x))
OR_group_up$scale_y<-(OR_group_up$y - min(OR_group_up$y)) / (max(OR_group_up$y) - min(OR_group_up$y))
Epha7_DV_p<-ggviolin(OR_group_up, x = "avg_exp_type", y = "scale_y", fill = "avg_exp_type", 
...
Epha7_AP_p<-ggviolin(OR_group_up, x = "avg_exp_type", y = "scale_x", fill = "avg_exp_type", 
              palette=c("#cdd2db","#45486f"), 
              ylim=c(0, 1),
              alpha = 1,width = 0.5,
              legend = "none",#去掉legend
              ...
              theme(plot.title = element_text(hjust = 0.5), 
                ...
                legend.title = element_blank(),
                text=element_text(size=8))
ggsave(filename = "/data/R02/huangtt39/ATAC-RNAseq/analysis/figure/Figure4/Epha7_OR_projection_vln.pdf",Epha7_DV_p+Epha7_AP_p, device = "pdf", width = 18, height = 10, units = "cm", dpi = 300)
