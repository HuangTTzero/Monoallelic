...
ORgene_FC_p <-ggplot(data=OR_count,aes(x=log2((Paternal_specific+0.1)/(Maternal_specific+0.1))))+
      ...
      coord_flip()
ggsave(filename = "/data/R02/huangtt39/ATAC-RNAseq/analysis/figure/Figure6/OR_cutoff.pdf",ORgene_cellnumber_p+plot_spacer()+ORgene_point_p+ORgene_FC_p+plot_layout(nrow=2,ncol=2,height=c(1,2,1,2),width=c(2,1,2,1)), device = "pdf", width =12, height = 9, units = "cm", dpi = 60,bg="white")
...
ggsave(filename =  "/data/R02/huangtt39/ATAC-RNAseq/analysis/figure/Figure6/OR_sample_distance_point.pdf",sample_distance_point_p+plot_layout(ncol=4,guides = 'collect'), device = "pdf", width =32, height = 8, units = "cm", dpi = 300,bg="white")
...
# Figure 6A-B S4C-D
OR_distance_significant<-OR_distance%>%subset(wilcox.test=="p.value<0.05")
OR_distance_significant_dscore<-OR_dscore%>%subset(gene%in%OR_distance_significant$gene)
...
distance_violin<-function(OR){
  ...
  boxplot_p<-ggviolin(plot_df, x = "group", y = "distance", fill = "#914350", 
              alpha = 1,width = 0.5,ylim=c(-0.5, 2.5),
...
OR_obj<-lapply(1:4,function(i){
  OR_cell<-OR_dscore%>%subset(gene==myOR_far[i])
...
# transcriptome difference VS SNP
...
write.table(avail_OR_CCDS%>%group_by(gene)%>%dplyr::slice(1),file="/data/R02/huangtt39/ATAC-RNAseq/analysis/merge/OR_annalysis/data/OR_CCDS_txid.txt",col.names = FALSE, row.names = FALSE,quote = FALSE,sep = "\t")

# select the ccds of each OR genes ,and remove the duplicates
avail_OR_CCDS <- unique(avail_OR_CCDS[,c("ccdsId","chrom","cdsStart","cdsEnd","gene")])# 73个gene remained，except for two ps1 gene
...
OR_distance<-OR_distance%>%mutate(wilcox.test=case_when(wilcox_p<0.05~"p.value<0.05",wilcox_p>=0.05~"p.value>=0.05"))
...
ggsave(filename = "/data/R02/huangtt39/ATAC-RNAseq/script_arrange/Figure1/unsyn_snp_distance.pdf",SNP_VS_length_p+distance_VS_snp_p+plot_layout(width=c(1,1.5)), device = "pdf", width = 21, height = 8, units = "cm", dpi = 60,bg="white")
...
OR_final_ccds<-read.table(file="/data/R02/huangtt39/ATAC-RNAseq/analysis/merge/OR_annalysis/data/OR_final_ccds.txt",header=TRUE)
...
,file="/data/R02/huangtt39/ATAC-RNAseq/analysis/merge/OR_annalysis/data/OR_classII_CCDS_txid.txt",col.names = FALSE, row.names = FALSE,quote = FALSE,sep = "\t")

# Import the constructed class II OR consensus sequence
classII_OR_consense<-read.xlsx("/data/R02/huangtt39/ATAC-RNAseq/analysis/merge/OR_annalysis/data/conservation.xlsx",sheet = 1)
