...
pdf("~/ATAC-RNAseq/analysis/merge/OR_annalysis/multiome_scRNA_mature_ORNs_UMIs_cutoff_tunning.pdf",width=8)
i <- "_1"
...
save(OR_dscore,file="~/F1_OSN/analysis/merge/OR_annalysis/OR_dscore.RData")
...
save(filtered_mature_OSNs,file="~/ATAC-RNAseq/analysis/merge/OR_annalysis/filtered_mature_OSNs.RData")
...
write.xlsx(OR_count,file="~/F1_OSN/analysis/figure/FigureS4/FigureS4a.xlsx")
...
ORgene_FC_p <-ggplot(data=OR_count,aes(x=log2((Paternal_specific+0.1)/(Maternal_specific+0.1))))+
      ...
      coord_flip()
ggsave(filename = "~/F1_OSN/analysis/figure/FigureS4/OR_cutoff.pdf",ORgene_cellnumber_p+plot_spacer()+ORgene_point_p+ORgene_FC_p+plot_layout(nrow=2,ncol=2,height=c(1,2,1,2),width=c(2,1,2,1)), device = "pdf", width =12, height = 9, units = "cm", dpi = 60,bg="white")
...
ggsave(filename =  "~/F1_OSN/analysis/figure/FigureS4/OR_sample_distance_point_seed12345.pdf",sample_distance_point_p+plot_layout(ncol=4,guides = 'collect'), device = "pdf", width =32, height = 8, units = "cm", dpi = 300,bg="white")
...
# 写入文件
write.xlsx(sample_distance, file = "~/F1_OSN/analysis/figure/FigureS4/FigureS4b.xlsx")
...
mm10_129_PWK_SNP <- read.table("~/F1_OSN/snp/pwk/pwk_specific.vcf",header=FALSE)[,c(2:6)]%>%rename_with(~c("chr","pos","id","ref","alt"),1:5)
...
txid2gene <- read.table("~/F1_OSN/mapping/reference/mm10-mask/genes/txid_to_name.txt")
...
write.table(avail_OR_CCDS%>%group_by(gene)%>%dplyr::slice(1),file="~/F1_OSN/analysis/merge/OR_annalysis/OR_CCDS_txid.txt",col.names = FALSE, row.names = FALSE,quote = FALSE,sep = "\t")
...
unsyn_variant<-read.table("~/F1_OSN/snp/snp_eff/pwk_specific/missense/pwk_specific_missense.txt")[,c(1,4)]
...
write.xlsx(OR_distance,file="~/F1_OSN/analysis/figure/Figure4/OR_distance_SNP.xlsx")
OR_distance<-read.xlsx("~/F1_OSN/analysis/figure/Figure4/OR_distance_SNP.xlsx")
...
ggsave(filename = "~/F1_OSN/analysis/figure/Figure4/OR_difference_ranking.pdf",OR_difference_ranking, device = "pdf", width=16, height = 8, units = "cm", dpi = 300,bg="white")
...
pdf("~/F1_OSN/analysis/figure/Figure4/OR_distance_heatmap.pdf",height=6.5,width=8.5)
...
pdf("~/F1_OSN/analysis/figure/Figure4/OR_distance_violin.pdf", height = 2.5, width = 3.5)
...
write.xlsx(do.call(rbind, all_stats), 
          file = "~/F1_OSN/analysis/figure/Figure4/OR_distance_statistics.xlsx",
          row.names = FALSE)

write.xlsx(all_distances, 
          file = "~/F1_OSN/analysis/figure/Figure4/OR_example_celldistances.xlsx",
          row.names = FALSE)
...
# }
# ggsave(filename = "~/F1_OSN/analysis/figure/Figure6/obj_temp_umap.pdf",umap_list[[1]]+umap_list[[2]]+umap_list[[3]]+umap_list[[4]]+plot_layout(ncol=2) & NoLegend() & theme(plot.title = element_text(hjust = 0.5,size=8),text=element_text(size=8),axis.text.y=element_text(size=8,color = "black"), axis.text.x=element_text(size=8,  color = "black")), device = "pdf", width =16, height = 16, units = "cm", dpi = 300)
...
saveWorkbook(wb, "~/F1_OSN/analysis/figure/Figure4/OR_differential_gene.xlsx", overwrite = TRUE)


# Figure 4c and Supplementary Figure 4c
...
ggsave(filename = "~/F1_OSN/analysis/figure/FigureS4c/OR_marker_Volcano_Plot.pdf",marker_p+plot_layout(ncol=4,guides="collect"), device = "pdf", width = 34, height = 8, units = "cm", dpi = 300,bg="white")
...
pdf("~/F1_OSN/analysis/figure/Figure4/OR_DEG_heat.pdf",height=4.5,width=8)
for(i in 1:3){
  OR_DEG_heat(i)
}
dev.off()
...
saveWorkbook(wb, "~/F1_OSN/analysis/figure/Figure4/OR_differential_peak.xlsx", overwrite = TRUE)
...
pdf("~/F1_OSN/analysis/figure/Figure4/OR_DEP_heat.pdf",height=4.5,width=6.5)
for(i in 1:3){
  OR_DEP_heat(i)
}
dev.off()
...
# Figure 4d & Supplementary Figure 4d-f
...
OR_dep_track<-function(i){
  ...
  exp<-ExpressionPlot(obj, features = peak[1,1],group.by="OR_allele_exptype", assay = "RNA")+scale_fill_manual(values=c("#b45a55","#505f84"))+
      theme(text=element_text(size=8))
  file=str_c("~/F1_OSN/analysis/merge/OR_annalysis/OR_gene_dep/",myOR_far[OR_index],"_",peak[1,1],"_",peak[1,2],".pdf")
...
ggsave(filename = "~/F1_OSN/analysis/figure/Figure4/unsyn_snp_distance.pdf",SNP_VS_length_p+distance_VS_snp_p+plot_layout(width=c(1,1.5)), device = "pdf", width = 21, height = 8, units = "cm", dpi = 60,bg="white")
...
load(file="~/F1_OSN/analysis/merge/OR_annalysis/final_OR_CCDS.RData")
OR_GPCR_ccds<-final_OR_CCDS%>%subset(gene%in%OR_distance_snp1$gene)%>%group_by(gene)%>%dplyr::slice(1)%>%select(gene,geneId)

# select class II OR
class_OR<-read.xlsx("~/F1_OSN/CODE/data/OSN/OR_class.xlsx",sheet = 2)
...
write.table(OR_GPCR_ccds%>%select(-2),file="~/F1_OSN/analysis/merge/OR_annalysis/GPCR/OR_GPCR_txid.txt",col.names = FALSE, row.names = FALSE,quote = FALSE,sep = "\t")
...
,file="~/F1_OSN/analysis/merge/OR_annalysis/OR_classII_CCDS_txid.txt",col.names = FALSE, row.names = FALSE,quote = FALSE,sep = "\t")
...
,file="~/F1_OSN/analysis/merge/OR_annalysis/OR_Olfr17_CCDS_txid.txt",col.names = FALSE, row.names = FALSE,quote = FALSE,sep = "\t")

# Import the constructed class II OR consensus sequence
classII_OR_consense<-read.xlsx("~/F1_OSN/CODE/data/GPCR/conservation.xlsx",sheet = 1)
...
ggsave(filename = "~/F1_OSN/analysis/merge/OR_annalysis/GPCR/GPCR_color.pdf",GPRC_p, device = "pdf", width = 30, height = 5, units = "cm", dpi = 300)
...
write.xlsx(classII_OR_consense,file="~/F1_OSN/analysis/figure/Figure5/GPCR/conservation2color.xlsx",sheet = 1)
...
#   theme_classic() +  # 使用简洁主题
#   scale_color_manual(values=c("#0E8A8F","#F44040"))
ggsave(filename = "~/F1_OSN/analysis/figure/Figure5/GPCR/distance_conservation.pdf",distance_conservation_p, device = "pdf", width = 10, height = 5, units = "cm", dpi = 300)

# Import the constructed class II OR consensus sequence
classII_OR_consense<-read.xlsx("~/F1_OSN/analysis/figure/Figure5/GPCR/conservation_add_stress.xlsx",sheet = 1)
...
ERstress_conservation_p<-ggplot(ERstress_conservation, aes(x = Erstress, y = Max.ratio)) +
              ...
              theme(plot.title = element_text(hjust = 0.5), 
                ...
                legend.title = element_blank(),
                text=element_text(size=6))
ggsave(filename = "~/F1_OSN/analysis/figure/FigureS4/GPCR/ERstress_conservation.pdf",ERstress_conservation_p, device = "pdf", width = 5, height = 5, units = "cm", dpi = 300)
