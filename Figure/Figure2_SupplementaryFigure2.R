...
write.table(data.frame(barcode=data1_mOSN),'~/F1_OSN/analysis/joint202201/OSN/joint202201_OSN_barcode.txt',sep = '\t', quote = FALSE, row.names = FALSE,col.names=FALSE)
#10294
# Load the data
dirs <- c("~/F1_OSN/mapping/joint202201/mapping_allele/joint202201_G1count/outs/","~/F1_OSN/mapping/joint202201/mapping_allele/joint202201_G2count/outs/","~/F1_OSN/mapping/joint202201/mapping_mask/joint202201_mask/outs/")
samples <- c("G1","G2","Ori")
...
save(objList,file="~/F1_OSN/analysis/joint202201/objList.RData")
...
write.table(fragmentobj[[1]], gzfile('~/F1_OSN/analysis/joint202201/pwk_fragment_anno.txt.gz'),sep = '\t', quote = FALSE, row.names = FALSE)
write.table(fragmentobj[[2]], gzfile('~/F1_OSN/analysis/joint202201/c57_fragment_anno.txt.gz'),sep = '\t', quote = FALSE, row.names = FALSE)
write.table(fragmentobj[[3]], gzfile('~/F1_OSN/analysis/joint202201/ori_fragment_anno.txt.gz'),sep = '\t', quote = FALSE, row.names = FALSE)
...
rm(fragmentobj)
save(fragcount,file="~/F1_OSN/analysis/joint202201/fragcount.RData")
...
stopCluster(cl)
save(readList,file="~/F1_OSN/analysis/joint202201/readList.RData")


# dataset2
data2_mOSN<-gsub('_2','',mOSN_barcode[grep('2$',mOSN_barcode)])
write.table(data.frame(barcode=data2_mOSN),'~/F1_OSN/analysis/jointF1OSN/OSN/jointF1OSN_OSN_barcode.txt',sep = '\t', quote = FALSE, row.names = FALSE,col.names=FALSE)
# 9960
# Load the data
dirs <- c("~/F1_OSN/mapping/jointF1OSN/mapping_allele/jointF1OSN_G1count/outs/","~/F1_OSN/mapping/jointF1OSN/mapping_allele/jointF1OSN_G2count/outs/","~/F1_OSN/mapping/jointF1OSN/mapping_mask/jointF1OSN_mask/outs/")
samples <- c("G1","G2","Ori")
objList <- lapply(1:length(dirs),function(i){
  counts <- Read10X_h5(str_c(dirs[i],"filtered_feature_bc_matrix.h5")) 
  fragpath <- str_c(dirs[i],"atac_fragments.tsv.gz")
...
save(objList,file="~/F1_OSN/analysis/jointF1OSN/objList.RData")
# fragment annotation of promoter of each gene
fragmentobj <- lapply(1:3,function(i){
  obj<-read_tsv(file=str_c(dirs[i],"atac_fragments.tsv.gz"),comment = "#",col_names =c("chr","start","end","barcode","count"))
  obj$peak<-paste(obj$chr,obj$start,obj$end,sep="-")
...
write.table(fragmentobj[[1]], gzfile('~/F1_OSN/analysis/jointF1OSN/pwk_fragment_anno.txt.gz'),sep = '\t', quote = FALSE, row.names = FALSE)
write.table(fragmentobj[[2]], gzfile('~/F1_OSN/analysis/jointF1OSN/c57_fragment_anno.txt.gz'),sep = '\t', quote = FALSE, row.names = FALSE)
write.table(fragmentobj[[3]], gzfile('~/F1_OSN/analysis/jointF1OSN/ori_fragment_anno.txt.gz'),sep = '\t', quote = FALSE, row.names = FALSE)
fragcount<-lapply(1:3,function(i){
...
rm(fragmentobj)
save(fragcount,file="~/F1_OSN/analysis/jointF1OSN/fragcount.RData")
###构建reads count objlist
library("parallel")
cl <- makeCluster(8)
...
stopCluster(cl)
save(readList,file="~/F1_OSN/analysis/jointF1OSN/readList.RData")


#dataset3
data3_mOSN<-gsub('_3','',mOSN_barcode[grep('3$',mOSN_barcode)])
write.table(data.frame(barcode=data3_mOSN),'~/F1_OSN/analysis/jointF18w/OSN/jointF18w_OSN_barcode.txt',sep = '\t', quote = FALSE, row.names = FALSE,col.names=FALSE)
# 9647 cells
# Load the data
dirs <- c("~/F1_OSN/mapping/jointF18w/mapping_allele/jointF18w_G1count/outs/","~/F1_OSN/mapping/jointF18w/mapping_allele/jointF18w_G2count/outs/","~/F1_OSN/mapping/jointF18w/mapping_mask/jointF18w_mask/outs/")
samples <- c("G1","G2","Ori")
objList <- lapply(1:length(dirs),function(i){
  counts <- Read10X_h5(str_c(dirs[i],"filtered_feature_bc_matrix.h5")) 
  fragpath <- str_c(dirs[i],"atac_fragments.tsv.gz")
...
save(objList,file="~/F1_OSN/analysis/jointF18w/objList.RData")
# fragment annotation of promoter of each gene
fragmentobj <- lapply(1:3,function(i){
  obj<-read_tsv(file=str_c(dirs[i],"atac_fragments.tsv.gz"),comment = "#",col_names =c("chr","start","end","barcode","count"))
  obj$peak<-paste(obj$chr,obj$start,obj$end,sep="-")
...
write.table(fragmentobj[[1]], gzfile('~/F1_OSN/analysis/jointF18w/pwk_fragment_anno.txt.gz'),sep = '\t', quote = FALSE, row.names = FALSE)
write.table(fragmentobj[[2]], gzfile('~/F1_OSN/analysis/jointF18w/c57_fragment_anno.txt.gz'),sep = '\t', quote = FALSE, row.names = FALSE)
write.table(fragmentobj[[3]], gzfile('~/F1_OSN/analysis/jointF18w/ori_fragment_anno.txt.gz'),sep = '\t', quote = FALSE, row.names = FALSE)
fragcount<-lapply(1:3,function(i){
...
rm(fragmentobj)
save(fragcount,file="~/F1_OSN/analysis/jointF18w/fragcount.RData")
###构建reads count objlist
library("parallel")
cl <- makeCluster(8)
...
stopCluster(cl)
save(readList,file="~/F1_OSN/analysis/jointF18w/readList.RData")
...
write.xlsx(rate_data, "~/F1_OSN/analysis/figure/FigureS2/FigureS2a.xlsx")
...
ggsave(filename = "~/F1_OSN/analysis/figure/FigureS2/reads.pdf",rate_p+p1+p2, device = "pdf", width = 30, height =10 , units = "cm", dpi = 60,bg="white")
write.xlsx(reads%>%select(position,values,ratio,dataset), "~/F1_OSN/analysis/figure/FigureS2/FigureS2b.xlsx")
...
pdf("~/F1_OSN/analysis/figure/FigureS2/reads_dis.pdf",width = 5, height =7, )
print(reads_dis_p)
dev.off()
reads_dis$ratio<-reads_dis$allele/reads_dis$total
write.xlsx(reads_dis, "~/F1_OSN/analysis/figure/FigureS2/FigureS2c.xlsx")
...
######合并数据#####
read_List<-list()
load(file="~/F1_OSN/analysis/joint202201/readList.RData")

read_List[[1]]<-readList
rm(readList)
load(file="~/F1_OSN/analysis/jointF1OSN/readList.RData")

read_List[[2]]<-readList
rm(readList)
load(file="~/F1_OSN/analysis/jointF18w/readList.RData")
...
# 2 Others   29019
# 3 chrX      1025
save(merge_count,file="~/F1_OSN/analysis/merge/RData/merge_count.RData")
...
save(dscore_data,file="~/F1_OSN/analysis/merge/RData/dscore_data.RData")
...
write.xlsx(merge_count2plot, "~/F1_OSN/analysis/figure/FigureS2/FigureS2d.xlsx")
...
ggsave(filename = "~/F1_OSN/analysis/figure/FigureS2/gene_count.pdf", merge_count2plot_p, device = "pdf", width = 12, height = 7, units = "cm", dpi = 300)
...
ggsave(filename = "~/F1_OSN/analysis/figure/FigureS2/cutoff_reads_both_genetype.pdf",cutoff_p(readscutoff_both%>%subset(type=="Others"))+cutoff_p(readscutoff_both%>%subset(type=="chrX"))+cutoff_p(readscutoff_both%>%subset(type=="OR"))&geom_vline(xintercept= c(log10(11)),linewidth=0.25,linetype="dashed"), device = "pdf", width =36, height = 6, units = "cm", dpi = 300,bg="white")
ggsave(filename = "~/F1_OSN/analysis/figure/FigureS2/cutoff_reads_gene_type.pdf",cutoff_genetype_p(readscutoff_rna)+cutoff_genetype_p(readscutoff_atac)+cutoff_genetype_p(readscutoff_both)+geom_vline(xintercept= c(log10(11)),linewidth=0.25,linetype="dashed"), device = "pdf", width = 20, height = 5, units = "cm", dpi = 300,bg="white")
ggsave(filename = "~/F1_OSN/analysis/figure/FigureS2/cutoff_reads_both_total.pdf",cutoff_p(readscutoff_both)+geom_vline(xintercept= c(log10(11)),linewidth=0.25,linetype="dashed"), device = "pdf", width =12, height = 6, units = "cm", dpi = 300,bg="white")
...
write.xlsx(horizontal_readscutoff_both, "~/F1_OSN/analysis/figure/FigureS2/FigureS2e.xlsx")
...
ggsave(filename = "~/F1_OSN/analysis/figure/FigureS2/final_gene.pdf",count_final_p,device = "pdf", width =10, height = 10, units = "cm", dpi = 300,bg="white")



####Smart-seq2 analysis
# Supplementary Figure 2f-k
genome <- 2725521370
genome_exon <- 110722912
...
write.xlsx(rate_data, "~/F1_OSN/analysis/figure/FigureS2/FigureS2f.xlsx")
...
ggsave(filename = "~/F1_OSN/analysis/figure/FigureS2/LiMCA_reads.pdf",rate_p+reads_vln_plot, device = "pdf", width = 30, height =10 , units = "cm", dpi = 60,bg="white")
write.xlsx(LiMCA_reads_long%>%select(Sample,Total_Fragments,strain,Group,Percentage), "~/F1_OSN/analysis/figure/FigureS2/FigureS2g.xlsx")
...
write.xlsx(LiMCA_merge_count2plot, "~/F1_OSN/analysis/figure/FigureS2/FigureS2h.xlsx")
...
ggsave(filename = "~/F1_OSN/analysis/figure/FigureS2/LiMCA_gene_count.pdf", LiMCA_merge_count2plot_p, device = "pdf", width = 8, height = 7, units = "cm", dpi = 300)
...
LiMCA_allele_meanRNA_p <-ggplot(data=LiMCA_merge_count%>%subset(rna_count>0),aes(x=log10(rna_mean)))+
      ...
      coord_flip()
ggsave(filename = "~/F1_OSN/analysis/figure/FigureS2/LiMCA_allele_count_meanRNA.pdf",LiMCA_allele_count_p+plot_spacer()+LiMCA_allele_count_meanRNA_p+LiMCA_allele_meanRNA_p+plot_layout(nrow=2,ncol=2,height=c(1,2,1,2),width=c(2,1,2,1)), device = "pdf", width = 10, height = 9, units = "cm", dpi = 300,bg="white")
write.xlsx(LiMCA_merge_count, "~/F1_OSN/analysis/figure/FigureS2/FigureS2i.xlsx")
...
horizontal_LiMCA_readscutoff <- LiMCA_readscutoff %>%
  select(gene, count, cutofftype) %>%
  pivot_wider(names_from = cutofftype, values_from = count)
write.xlsx(horizontal_LiMCA_readscutoff, "~/F1_OSN/analysis/figure/FigureS2/FigureS2j.xlsx")
...
ggsave(filename = "~/F1_OSN/analysis/figure/FigureS2/LiMCA_cutoff_reads_total.pdf",cutoff_p(LiMCA_readscutoff)+geom_vline(xintercept= c(log10(11)),linewidth=0.25,linetype="dashed"), device = "pdf", width =12, height = 6, units = "cm", dpi = 300,bg="white")
...
write.xlsx(LiMCA_to_Positive, "~/F1_OSN/analysis/figure/FigureS2/FigureS2k.xlsx")
...
# the expression pattern of each gene 
# add positive control, imprinting gene
imprint<-read.xlsx("~/F1_OSN/CODE/data/mouse/imprint_new.xlsx",sheet = 1)
...
ggsave(filename ="~/F1_OSN/analysis/figure/Figure1/Positve.pdf",Positve_p, device = "pdf", width = 10, height =7, units = "cm", dpi = 300,bg="white")
...
save(sample_100,file="~/F1_OSN/analysis/merge/RData/sample_100.RData")
...
write.xlsx(final_data,file="~/F1_OSN/analysis/figure/Figure2/Figure2c.xlsx")
...
save(sample_100_RNA2,file="~/F1_OSN/analysis/merge/RData/sample_100_RNA2.RData")
...
write.xlsx(plot_data_others_toxlsx,file="~/F1_OSN/analysis/figure/Figure2/Figure2d.xlsx")
...
library(gridExtra)
# 4. 创建PDF文件
pdf("~/F1_OSN/analysis/figure/LiMCA/scatter_plots_mono.pdf", width = 23, height = 4)
...
save(final_count_ratio,file="~/F1_OSN/analysis/merge/RData/final_count_ratio.RData")
write.xlsx(final_count_ratio,file="~/F1_OSN/analysis/figure/Figure2/Figure2be.xlsx")
...
# Same layout as Fig. 2e; y = -log10(P) on log scale so extreme P values
# do not compress low-significance genes. Axis labels show raw -log10(P).
count_pval_cutoff_p <- ggplot() +
  geom_point(data = final_count_ratio,
             aes(x = count,
                 y = pmax(-log10(pmax(p_value, .Machine$double.xmin)), 0.1),
                 color = color_group),
             size = 1, alpha = 0.9) +
  ...
  theme(plot.title = element_text(hjust = 0.5),
        legend.position = "right",
        text = element_text(size = 10))
ggsave(filename = "~/F1_OSN/analysis/figure/Figure2/count_vs_neglog10p.pdf",
       count_pval_cutoff_p, device = "pdf", width = 12, height = 7, units = "cm", dpi = 300, bg = "white")
...
sample_count <- sample_count %>%
  mutate(
    Maternal_specific_ratio = Maternal_specific / count,
    Paternal_specific_ratio = Paternal_specific / count,
    Biallelic_ratio = Biallelic / count
  )
write.xlsx(sample_count,file="~/F1_OSN/analysis/figure/Figure2/permutation_gene_count.xlsx")

sample_count%>%subset(q_value<0.05) #数据为0

ggsave(filename = "~/F1_OSN/analysis/figure/Figure2/get_nobiallelic_gene.pdf",count_ratio_cutoff_p+Total_ratio_p+plot_layout(ncol=2,widths=c(1.5,1)), device = "pdf", width = 20, height = 7, units = "cm", dpi = 300,bg="white")

# Fig. 2e axes: no_biallelic genes (type colors) + permutation null (hex density)
if (!exists("final_count_ratio")) {
  load("~/F1_OSN/analysis/merge/RData/final_count_ratio.RData")
}
if (!exists("cellcolor")) {
  cellcolor <- c("#597b83", "#d8bf87", "#d9c3c5")
}
# final_count_ratio: Maternal/Paternal are ratios; keep a count-table alias only for gene lists / q_value
if (!exists("final_count")) {
  final_count <- final_count_ratio %>%
    dplyr::distinct(gene, type, count, p_value, q_value, .keep_all = TRUE) %>%
    mutate(
      # if Maternal/Paternal are ratios (from ratio table), restore cell counts for pbinom-style columns
      Maternal_specific_n = round(Maternal_specific * count),
      Paternal_specific_n = round(Paternal_specific * count),
      monoallelic_count = Maternal_specific_n + Paternal_specific_n
    )
}
if (!exists("no_biallelic")) {
  no_biallelic <- final_count %>% dplyr::filter(q_value < 0.05)
}
if (!exists("no_biallelic_ratio")) {
  no_biallelic_ratio <- final_count_ratio %>% dplyr::filter(gene %in% no_biallelic$gene)
}

sample_count <- read.xlsx(
  "~/F1_OSN/analysis/figure/Figure2/permutation_gene_count.xlsx",
  sheet = 1
)
fig2e_type_colors <- c(
  "OR" = cellcolor[1],
...
perm_fig2e_data <- sample_count %>%
  mutate(Parental_specific = Maternal_specific_ratio + Paternal_specific_ratio)
no_biallelic_fig2e <- no_biallelic_ratio %>%
  mutate(Parental_specific = Maternal_specific + Paternal_specific)

perm_hex_bins <- 35
# log10 scale: count=1 (~white) -> high count (dark blue)
perm_fill_colors <- c("#ffffff", "#f5f8fc", "#e8eef6", "#c5d4e8", "#7a9bc4", "#3a6fa8", "#1a3d5c")
perm_overlay_base <- list(
  theme = theme_classic() +
    theme(plot.title = element_text(hjust = 0.5),
          legend.position = "right",
          text = element_text(size = 10)),
  xlab = "# cell of each gene (log10)",
  ylab = "% Parental specific cell"
)

# Version 1: hex bin, log10-scaled permutation count (low ~ white, high ~ dark)
nobiallelic_perm_overlay_hex_log10_p <- ggplot() +
  geom_hex(
    data = perm_fig2e_data,
    aes(x = log10(count), y = Parental_specific),
    bins = perm_hex_bins,
    alpha = 0.6
  ) +
  scale_fill_gradientn(
    colours = perm_fill_colors,
    trans = "log10",
    limits = c(1, NA),
    oob = scales::squish,
    name = "Permutation\ncount"
  ...
  geom_point(
    data = no_biallelic_fig2e,
    aes(x = log10(count), y = Parental_specific, color = type),
    size = 1.2,
    alpha = 0.95,
    inherit.aes = FALSE
  ) +
  scale_color_manual(values = fig2e_type_colors, name = "Gene Type") +
  guides(fill = guide_colorbar(order = 1), color = guide_legend(order = 2)) +
  scale_x_continuous(limits = c(log10(10), NA)) +
  scale_y_continuous(limits = c(0, 1)) +
  labs(title = "Hex (log10 count)",
       x = perm_overlay_base$xlab, y = perm_overlay_base$ylab) +
  perm_overlay_base$theme

ggsave(
  filename = "~/F1_OSN/analysis/figure/Figure2/no_biallelic_perm_overlay_hex_log10.pdf",
  nobiallelic_perm_overlay_hex_log10_p,
  device = "pdf", width = 10.5, height = 8, units = "cm", dpi = 300, bg = "white"
)
# # Version 2: hex bin, log2-scaled permutation count (low ~ white, high ~ dark)
# nobiallelic_perm_overlay_hex_log2_p <- ggplot() +
#   geom_hex(
#     data = perm_fig2e_data,
#     aes(x = log10(count), y = Parental_specific),
#     bins = perm_hex_bins,
...
#   scale_x_continuous(limits = c(log10(10), NA)) +
#   scale_y_continuous(limits = c(0, 1)) +
#   labs(title = "Hex (log2 count)",
#        x = perm_overlay_base$xlab, y = perm_overlay_base$ylab) +
#   perm_overlay_base$theme

# nobiallelic_perm_overlay_combined_p <- patchwork::wrap_plots(
#   nobiallelic_perm_overlay_hex_log10_p,
#   nobiallelic_perm_overlay_hex_log2_p,
#   ncol = 2
# )
# ggsave(
#   filename = "~/F1_OSN/analysis/figure/Figure2/no_biallelic_perm_overlay.pdf",
#   nobiallelic_perm_overlay_combined_p,
#   device = "pdf", width = 21, height = 8, units = "cm", dpi = 300, bg = "white"
# )

# Version 3: raw permutation points (dot) instead of hex density
# nobiallelic_perm_overlay_dot_p <- ggplot() +
#   geom_point(
#     data = perm_fig2e_data,
#     aes(x = log10(count), y = Parental_specific),
#     color = "#7a9bc4",
...
#   scale_y_continuous(limits = c(0, 1)) +
#   labs(
#     title = "Permutation (raw dots)",
#     x = perm_overlay_base$xlab,
#     y = perm_overlay_base$ylab
#   ) +
#   perm_overlay_base$theme

# ggsave(
#   filename = "~/F1_OSN/analysis/figure/Figure2/no_biallelic_perm_overlay_dot.pdf",
#   nobiallelic_perm_overlay_dot_p,
#   device = "pdf",
#   width = 12,
#   height = 8,
#   units = "cm",
...
# Permutation percentile: observed monoallelic ratio vs 100 permutations per gene
others_control_genes <- final_count %>%
  dplyr::filter(type == "Others", !gene %in% c("Peg3", "Kcnq1ot1"), !gene %in% candicate_gene$gene) %>%
  left_join(final_count_ratio %>% dplyr::select(gene, q_value), by = "gene") %>%
  dplyr::filter(is.na(q_value) | q_value >= 0.05)
# Observed monoallelic fraction: use final_count_ratio (Maternal/Paternal are already ratios).
# Do NOT divide by count again. Native final_count (L1582) stores cell numbers; that path uses monoallelic_count/count.
observed_monoallelic <- final_count_ratio %>%
  mutate(monoallelic_ratio = Maternal_specific + Paternal_specific) %>%
  dplyr::select(gene, monoallelic_ratio)
perm_monoallelic <- sample_count %>%
  mutate(monoallelic_ratio = (Maternal_specific + Paternal_specific) / count)

calc_perm_exceed_n <- function(gene_name) {
  obs <- observed_monoallelic$monoallelic_ratio[observed_monoallelic$gene == gene_name]
  perms <- perm_monoallelic$monoallelic_ratio[perm_monoallelic$gene == gene_name]
  if (length(obs) == 0 || length(perms) == 0 || is.na(obs)) {
    return(NA_integer_)
  }
  sum(perms < obs, na.rm = TRUE)
}

perm_percentile_compare <- bind_rows(
  candicate_gene %>% mutate(group = "non-OR_RME"),
  others_control_genes %>% mutate(group = "Others (non-sig.)")
) %>%
  distinct(gene, group) %>%
  rowwise() %>%
  mutate(
    monoallelic_ratio = observed_monoallelic$monoallelic_ratio[match(gene, observed_monoallelic$gene)],
    perm_exceed_n = calc_perm_exceed_n(gene),
    perm_percentile = perm_exceed_n
  ) %>%
  ungroup() %>%
  dplyr::filter(!is.na(perm_exceed_n)) %>%
  mutate(perm_percentile_frac = perm_exceed_n / 100)
perm_percentile_compare$group <- factor(
  perm_percentile_compare$group,
  levels = c("non-OR_RME", "Others (non-sig.)")
)
write.xlsx(perm_percentile_compare, "~/F1_OSN/analysis/figure/Figure2/perm_percentile_compare.xlsx")

library(ggpubr)
perm_percentile_density_p <- ggplot(perm_percentile_compare, aes(x = perm_percentile_frac, fill = group, color = group)) +
  geom_histogram(
  ...
  scale_fill_manual(values = c("non-OR_RME" = "#828e79", "Others (non-sig.)" = "#b8c6de")) +
  scale_color_manual(values = c("non-OR_RME" = "#5f6e5a", "Others (non-sig.)" = "#6a8aab")) +
  scale_x_continuous(limits = c(0, 1), breaks = seq(0, 1, 0.25)) +
  labs(x = "Percentile fraction", y = "Density", fill = "Group", color = "Group") +
  annotate(
    "text",
    x = 0.5,
    y = Inf,
    label = paste0(
      "p = ",
      format.pval(
        wilcox.test(perm_percentile_frac ~ group, data = perm_percentile_compare, exact = FALSE)$p.value,
        digits = 2,
        eps = 1e-300
      )
    ),
...
ggsave(
  filename = "~/F1_OSN/analysis/figure/Figure2/perm_percentile_density.pdf",
  perm_percentile_density_p,
  device = "pdf",
  width = 8,
  height = 6,
  units = "cm",
...
# ECDF is more appropriate for discrete percentile fractions
perm_percentile_ecdf_p <- ggplot(perm_percentile_compare, aes(x = perm_percentile_frac, color = group)) +
  stat_ecdf(linewidth = 0.8, pad = FALSE, geom = "step") +
  scale_color_manual(values = c("non-OR_RME" = "#5f6e5a", "Others (non-sig.)" = "#6a8aab")) +
  scale_x_continuous(limits = c(0, 1), breaks = seq(0, 1, 0.25)) +
  scale_y_continuous(limits = c(0, 1), breaks = seq(0, 1, 0.25)) +
  labs(x = "Percentile fraction", y = "Cumulative fraction", color = "Group") +
  theme_classic() +
  theme(
    legend.title = element_blank(),
    text = element_text(size = 8)
...
ggsave(
  filename = "~/F1_OSN/analysis/figure/Figure2/perm_percentile_ecdf.pdf",
  perm_percentile_ecdf_p,
  device = "pdf",
  width = 8,
  height = 6,
  units = "cm",
...
# permutation 1000 times for candidate genes (same structure as sample_count)
candicate_perm_n <- 1000L
candicate_perm_seed <- 20260823L

# test_cell_sample_candicate <- function(temp_dscore) {
#   cell_sample <- function(x) {
#     G1 <- rbinom(candicate_perm_n, size = x, p = 0.5)
#     G2 <- x - G1
#     return(cbind(G1, G2, seq_len(candicate_perm_n)))
#   }
#   sample_rna <- map(temp_dscore$allele_exp, cell_sample)
#   sample_atac <- map(temp_dscore$allele_frag, cell_sample)
#   sample_rna <- do.call(rbind, sample_rna) %>% as.data.frame()
...
# get_sample_candicate <- function(genename) {
#   temp_dscore <- final_gene_dscore %>% subset(gene == genename)
#   sample_perm <- test_cell_sample_candicate(temp_dscore)
#   sample_perm$gene <- rep(genename, nrow(sample_perm))
#   return(sample_perm)
# }

set.seed(candicate_perm_seed)
cl <- makeCluster(16)
clusterSetRNGStream(cl, iseed = candicate_perm_seed)
clusterExport(cl, c("final_gene_dscore", "get_sample_candicate", "test_cell_sample_candicate", "candicate_perm_n"))
candicate_sample_1000 <- parLapply(cl, candicate_gene$gene, function(genename) {
  library(dplyr)
  library(purrr)
  get_sample_candicate(genename)
})
stopCluster(cl)
save(candicate_sample_1000 , file = "~/F1_OSN/analysis/merge/RData/candicate_sample_1000.RData")

# Per-gene aggregation avoids OOM from rbind + mutate on the full cell x round table
candicate_sample_celltype_count <- do.call(rbind, lapply(candicate_sample_1000, function(sample_perm) {
  sample_perm$mergescore <- (sample_perm$G1rna + sample_perm$G1peak) /
    (sample_perm$G1rna + sample_perm$G1peak + sample_perm$G2rna + sample_perm$G2peak) - 0.5
  sample_perm <- sample_perm %>%
    mutate(merge_dscore_type = case_when(
      mergescore == -0.5 ~ "Maternal_specific",
      mergescore > (-0.5) & mergescore < 0.5 ~ "Biallelic",
      mergescore == 0.5 ~ "Paternal_specific"
    ))
  sample_perm %>%
    group_by(gene, merge_dscore_type, round) %>%
    summarise(cellnumber = n(), .groups = "drop")
}))
rm(candicate_sample_1000)
gc(verbose = FALSE)

candicate_sample_celltype_count <- merge(
  candicate_sample_celltype_count,
  candicate_gene %>% dplyr::select(gene, type, count),
  by = "gene"
)
candicate_sample_count <- candicate_sample_celltype_count %>%
  mutate(ratio = cellnumber / count) %>%
  pivot_wider(
    names_from = "merge_dscore_type",
    values_from = "cellnumber",
    id_cols = c(type, gene, count, round),
    values_fill = 0
  )

# 对 permutation 结果计算 p_value 和 q_value（按 round 分组校正）
candicate_sample_count <- candicate_sample_count %>%
  group_by(round) %>%
  mutate(
    monoallelic_count = Maternal_specific + Paternal_specific,
    p_value = pbinom(monoallelic_count - 1, size = count, prob = 0.125, lower.tail = FALSE),
...
save(candicate_sample_count, file = "~/F1_OSN/analysis/merge/RData/candicate_sample_count.RData")
write.xlsx(
  candicate_sample_count,
  "~/F1_OSN/analysis/figure/Figure2/candicate_permutation_gene_count.xlsx"
)

# Gene-specific observed fraction vs gene-specific permutation distribution
candicate_perm_monoallelic <- candicate_sample_count %>%
  mutate(monoallelic_ratio = (Maternal_specific + Paternal_specific) / count)

candicate_gene_perm_long <- candicate_perm_monoallelic
candicate_gene_obs <- observed_monoallelic %>%
  dplyr::filter(gene %in% candicate_gene$gene) %>%
  left_join(candicate_gene %>% dplyr::select(gene, count), by = "gene") %>%
  rename(obs_fraction = monoallelic_ratio)

# Empirical p from permutation distribution (one-sided: higher monoallelic = more extreme)
# p = (1 + #{perm >= obs}) / (1 + n_perm); percentile = #{perm < obs} / n_perm
candicate_gene_perm_pvalue <- candicate_gene_perm_long %>%
  left_join(candicate_gene_obs %>% dplyr::select(gene, obs_fraction), by = "gene") %>%
  group_by(gene) %>%
  summarise(
    n_perm = dplyr::n(),
    obs_fraction = dplyr::first(obs_fraction),
    perm_exceed_n = sum(monoallelic_ratio >= obs_fraction, na.rm = TRUE),
    perm_below_n = sum(monoallelic_ratio < obs_fraction, na.rm = TRUE),
    perm_percentile = perm_below_n / n_perm,
    p_perm = (1 + perm_exceed_n) / (1 + n_perm),
    .groups = "drop"
  ) %>%
  mutate(q_perm = p.adjust(p_perm, method = "BH")) %>%
  arrange(p_perm, q_perm)

candicate_gene_perm_ns <- candicate_gene_perm_pvalue %>%
  dplyr::filter(q_perm > 0.05)
message(
  "Candidate genes with permutation q_perm > 0.05: ",
  nrow(candicate_gene_perm_ns), " / ", nrow(candicate_gene_perm_pvalue)
)
print(candicate_gene_perm_ns %>% dplyr::select(gene, obs_fraction, perm_percentile, p_perm, q_perm))
write.xlsx(
  candicate_gene_perm_pvalue,
  "~/F1_OSN/analysis/figure/Figure2/candicate_gene_perm_pvalue.xlsx"
)


candicate_gene_perm_summary <- candicate_gene_perm_long %>%
  group_by(gene) %>%
  summarise(
    perm_mean = mean(monoallelic_ratio, na.rm = TRUE),
    perm_max = max(monoallelic_ratio, na.rm = TRUE),
    perm_median = median(monoallelic_ratio, na.rm = TRUE),
    perm_q01 = quantile(monoallelic_ratio, 0.01, na.rm = TRUE),
    perm_q99 = quantile(monoallelic_ratio, 0.99, na.rm = TRUE),
    .groups = "drop"
  ) %>%
  left_join(candicate_gene_obs %>% dplyr::select(gene, obs_fraction, count), by = "gene") %>%
  left_join(
    candicate_gene_perm_pvalue %>% dplyr::select(gene, perm_percentile, p_perm, q_perm),
    by = "gene"
  ) %>%
  mutate(perm_diff = obs_fraction - perm_max) %>%
  arrange(perm_diff)

candicate_gene_order <- candicate_gene_perm_summary$gene

candicate_gene_perm_summary$gene <- factor(candicate_gene_perm_summary$gene, levels = candicate_gene_order)
candicate_gene_perm_long$gene <- factor(candicate_gene_perm_long$gene, levels = candicate_gene_order)
candicate_gene_obs$gene <- factor(candicate_gene_obs$gene, levels = candicate_gene_order)

candicate_gene_perm_summary <- candicate_gene_perm_summary %>%
  left_join(final_count %>% dplyr::select(gene, p_value, q_value), by = "gene") %>%
  mutate(
    obs_neg_log10_p = -log10(pmax(p_value, .Machine$double.xmin)),
    obs_log2_neg_log10_p = log2(pmax(obs_neg_log10_p, 0.1))
  ) %>%
  arrange(perm_diff)
candicate_gene_perm_summary$gene <- factor(candicate_gene_perm_summary$gene, levels = candicate_gene_order)

candicate_gene_perm_pval_long <- candicate_sample_count %>%
  mutate(
    neg_log10_p = -log10(pmax(p_value, .Machine$double.xmin)),
    log2_neg_log10_p = log2(pmax(neg_log10_p, 0.1))
  )
candicate_gene_perm_pval_summary <- candicate_gene_perm_pval_long %>%
  group_by(gene) %>%
  summarise(
    perm_p_q01 = quantile(log2_neg_log10_p, 0.01, na.rm = TRUE),
    perm_p_q99 = quantile(log2_neg_log10_p, 0.99, na.rm = TRUE),
    .groups = "drop"
  ) %>%
  left_join(candicate_gene_perm_summary %>% dplyr::select(gene, obs_log2_neg_log10_p), by = "gene")

candicate_gene_perm_pval_long$gene <- factor(
  candicate_gene_perm_pval_long$gene,
  levels = candicate_gene_order
)
candicate_gene_perm_pval_summary$gene <- factor(
  candicate_gene_perm_pval_summary$gene,
  levels = candicate_gene_order
)
write.xlsx(candicate_gene_perm_summary, "~/F1_OSN/analysis/figure/Figure2/candicate_gene_perm_summary.xlsx")

candicate_gene_perm_x_scale <- scale_x_discrete(
  limits = candicate_gene_order,
  expand = expansion(mult = c(0.01, 0.01))
)
candicate_perm_legend_interval <- "Permutation fraction (1%-99%)"
candicate_perm_legend_interval_pval <- "Permutation (1%-99%)"
candicate_perm_legend_obs <- "Observed"
candicate_perm_legend_theme <- theme(
  legend.position = "right",
  legend.title = element_blank(),
  legend.key.size = unit(0.4, "cm"),
  legend.text = element_text(size = 7)
)
candicate_perm_color_scale <- scale_color_manual(
  values = stats::setNames(
    c("#6a8aab", "#828e79"),
    c(candicate_perm_legend_interval, candicate_perm_legend_obs)
  ),
  breaks = c(candicate_perm_legend_interval, candicate_perm_legend_obs),
  name = NULL
)
candicate_perm_color_guides <- guides(
  color = guide_legend(
    override.aes = list(
      linewidth = c(1.2, 0),
      linetype = c(1, 0),
...
candicate_perm_color_scale_pval <- scale_color_manual(
  values = stats::setNames(
    c("#6a8aab", "#828e79"),
    c(candicate_perm_legend_interval_pval, candicate_perm_legend_obs)
  ),
  breaks = c(candicate_perm_legend_interval_pval, candicate_perm_legend_obs),
  name = NULL
)

candicate_gene_perm_bar_p <- ggplot(
  candicate_gene_perm_summary,
  aes(x = gene, y = count)
) +
  geom_col(fill = "#c5d4e8", width = 0.85, linewidth = 0.15, color = "grey75") +
  candicate_gene_perm_x_scale +
  labs(x = NULL, y = "# cells") +
  theme_classic() +
  theme(
...
candicate_gene_perm_interval_panel_p <- ggplot() +
  geom_violin(
    data = candicate_gene_perm_long,
    aes(x = gene, y = monoallelic_ratio),
    fill = "#d8e2f0",
    color = "grey70",
    scale = "width",
    alpha = 0.85,
  ...
  geom_linerange(
    data = candicate_gene_perm_summary,
    aes(
      x = gene,
      ymin = perm_q01,
      ymax = perm_q99,
      color = candicate_perm_legend_interval
    ),
    linewidth = 0.25
  ) +
  geom_point(
    data = candicate_gene_obs,
    aes(x = gene, y = obs_fraction, color = candicate_perm_legend_obs),
    size = 0.9
  ) +
  candicate_gene_perm_x_scale +
  candicate_perm_color_scale +
  candicate_perm_color_guides +
  scale_y_continuous(limits = c(0, 1), breaks = seq(0, 1, 0.25)) +
  labs(
    x = "Candidate genes (ranked by observed - permutation max)",
    y = "Monoallelic fraction",
    caption = "Blue bar: permutation monoallelic fraction (1%-99%); gray dot: observed monoallelic fraction"
  ) +
  theme_classic() +
  candicate_perm_legend_theme +
  theme(
    axis.text.x = element_blank(),
    axis.ticks.x = element_blank(),
    text = element_text(size = 8),
    plot.caption = element_text(size = 6.5, hjust = 0, color = "grey30")
  )

candicate_gene_perm_interval_p <- patchwork::wrap_plots(
  candicate_gene_perm_bar_p,
  candicate_gene_perm_interval_panel_p,
  ncol = 1,
  heights = c(0.28, 0.72),
  guides = "collect"
) & theme(plot.margin = margin(2, 4, 2, 4, "pt"))
ggsave(
  filename = "~/F1_OSN/analysis/figure/Figure2/candicate_gene_perm_interval.pdf",
  candicate_gene_perm_interval_p,
  device = "pdf",
  width = 24,
  height = 14,
  units = "cm",
...
candicate_gene_perm_summary%>%subset(obs_fraction <= perm_max)
# 13
...
ggsave(filename = "~/F1_OSN/analysis/figure/Figure2/nobiallelic_gene.pdf",candicate_gene_p, device = "pdf", width = 8, height = 7, units = "cm", dpi = 300,bg="white")
...
write.xlsx(gene_consistency_modified,file="~/F1_OSN/analysis/figure/Figure2/Figure2g.xlsx")
...
# 3. 如果需要保存图片
ggsave(filename = "~/F1_OSN/analysis/figure/Figure2/gene_consistency_violin.pdf", 
       consistency_vlnplot, 
       device = "pdf", 
       width = 10, 
       height = 6, 
...
rm(fragcount)
save(fragment,file="~/F1_OSN/analysis/merge/fragment.RData")
...
save(dist_data,file="~/F1_OSN/new_analysis/dist.RData")
...
write.xlsx(no_biallelic_ratio,file="~/F1_OSN/analysis/figure/Figure2/Figure2f.xlsx")
write.xlsx(no_biallelic,file="~/F1_OSN/analysis/figure/Figure2/no_biallelic.xlsx")
write.xlsx(RME,file="~/F1_OSN/analysis/figure/Figure2/Figure2h.xlsx")
...
ggsave(filename = "~/F1_OSN/analysis/figure/Figure2/dist.pdf",dist_p, device = "pdf", width = 11, height = 3, units = "cm", dpi = 300,bg="white")
