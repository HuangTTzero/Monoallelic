...
ggsave(filename = "~/F1_OSN/analysis/figure/Figure8/high_RME_OR_CV_new.pdf",high_RME_OR_up_exp_p, device = "pdf", width = 13, height = 7, units = "cm", dpi = 300)
...
pdf("~/F1_OSN/new_analysis/CV_violin.pdf",height=2.5,width=3.5)
DS_gene<-high_RME_OR_CV%>%subset(cv>=53.48619&f_test_p<0.05) # 5 genes remained
for(gene in DS_gene$gene){
  CV_violin(gene)
}
dev.off()

high_RME_OR_CV$f_test_p <- pmin(
  1,
  2 * pmin(
    pf(high_RME_OR_CV$f_statistic, high_RME_OR_CV$df1, high_RME_OR_CV$df2, lower.tail = TRUE),
    pf(high_RME_OR_CV$f_statistic, high_RME_OR_CV$df1, high_RME_OR_CV$df2, lower.tail = FALSE)
  )
)
high_RME_OR_CV$f_test_p_sci <- format(high_RME_OR_CV$f_test_p, scientific = TRUE, digits = 3)
write.xlsx(
  high_RME_OR_CV,
  file = "~/F1_OSN/analysis/figure/Figure8/Figure8ab.xlsx"
)
...
# Figure 8d
gene_disease<-read.xlsx("~/F1_OSN/CODE/data/ds_gene/human_CNV_HPO_gene.xlsx")[,c(15:17,19)]
...
Human_Mouse<-read.table(file="~/F1_OSN/CODE/data/ds_gene/Human_Mouse.txt",sep="\t",header=TRUE)[,c(4,10)]
Human_Mouse_subset<-Human_Mouse%>%subset(Gene.name%in%gene)%>%arrange(Gene.name)

Dsgene<-read.xlsx("~/F1_OSN/CODE/data/ds_gene/human_dosage_sensitivity.xlsx")[,c(1:3)]
...
write.xlsx(Dsgene_RME,file="~/F1_OSN/analysis/figure/Figure8/Figure8d.xlsx")
...
Dsgene_dot<-ggplot(data=Dsgene_RME_longer,aes(y=Mouse.gene.name,x=score_name,size=score,fill=score_name,color=score_name))+
  ...
  theme(plot.title = element_text(hjust = 0.5), 
        ...
        legend.title = element_blank(),
        text=element_text(size=8))
ggsave(filename = "~/F1_OSN/analysis/figure/Figure8/Dsgene_dot.pdf",Dsgene_dot, device = "pdf", width = 8, height =8, units = "cm", dpi = 300)
