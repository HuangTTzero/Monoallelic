...
# no result with BP and MF

RME_GO_p <- ggplot(data = RME_GOenrichment_result, 
                   aes(x = Description, y = Count, fill = -log10(p.adjust))) +
  geom_bar(stat = "identity", width = 0.8) +
  coord_flip() +
  scale_fill_gradientn(
    name = "-log10(FDR)",
    colors = c("#f2f7e8", "#D1Dbbd", "#3E606F", "#193441"),
    limits = c(1, 3)  # 固定颜色范围从1到3
  ) +
  labs(x = "GO term", y = "Gene Number", 
       title = "GO enrichment of RME genes in the Cellular Component category") +
  theme_classic() +
  theme(
...
ggsave(filename = "/data/R02/huangtt39/F1_OSN/new_analysis/RME_GO.pdf", 
       RME_GO_p, device = "pdf", width = 15, height = 10, units = "cm", 
       dpi = 300, bg = "white")
