...
library("bedtoolsr")
library("parallel")
load("~/F1_OSN/F1joint_mask/combined.RData")
...
p2<-ggplot(reads[[2]]) +
  ...
  coord_polar(theta = "y") + # 转换坐标轴
  theme_void() # 隐藏图例
ggsave(filename = "/data/R02/huangtt39/F1_OSN/analysis/figure/reads.pdf",rate_p+p1+p2, device = "pdf", width = 30, height =10 , units = "cm", dpi = 60,bg="white")
...
pdf("/data/R02/huangtt39/F1_OSN/analysis/figure/Figure1/reads_dis.pdf",width = 5, height =7, )
print(reads_dis_p)
dev.off()
...
# 2 Others   29019
# 3 chrX      1025
save(merge_count,file="/data/R02/huangtt39/F1_OSN/analysis/merge/merge_count.RData")
...
ggsave(filename = "/data/R02/huangtt39/F1_OSN/analysis/figure/Figure1/gene_count.pdf", merge_count2plot_p, device = "pdf", width = 12, height = 7, units = "cm", dpi = 300)
...
ggsave(filename = "/data/R02/huangtt39/F1_OSN/analysis/figure/Figure1/cutoff_reads_both_genetype.pdf",cutoff_p(readscutoff_both%>%subset(type=="Others"))+cutoff_p(readscutoff_both%>%subset(type=="chrX"))+cutoff_p(readscutoff_both%>%subset(type=="OR"))&geom_vline(xintercept= c(log10(11)),linewidth=0.25,linetype="dashed"), device = "pdf", width =36, height = 6, units = "cm", dpi = 300,bg="white")
ggsave(filename = "/data/R02/huangtt39/F1_OSN/analysis/figure/Figure1/cutoff_reads_gene_type.pdf",cutoff_genetype_p(readscutoff_rna)+cutoff_genetype_p(readscutoff_atac)+cutoff_genetype_p(readscutoff_both)+geom_vline(xintercept= c(log10(11)),linewidth=0.25,linetype="dashed"), device = "pdf", width = 20, height = 5, units = "cm", dpi = 300,bg="white")
ggsave(filename = "/data/R02/huangtt39/F1_OSN/analysis/figure/Figure1/cutoff_reads_both_total.pdf",cutoff_p(readscutoff_both)+geom_vline(xintercept= c(log10(11)),linewidth=0.25,linetype="dashed"), device = "pdf", width =12, height = 6, units = "cm", dpi = 300,bg="white")
...
ggsave(filename = "/data/R02/huangtt39/F1_OSN/analysis/figure/Figure1/final_gene.pdf",count_final_p,device = "pdf", width =10, height = 10, units = "cm", dpi = 300,bg="white")
...
ggsave(filename = "/data/R02/huangtt39/F1_OSN/new_analysis/get_nobiallelic_gene.pdf",count_ratio_cutoff_p+Total_ratio_p+plot_layout(ncol=2,widths=c(1.5,1)), device = "pdf", width = 20, height = 7, units = "cm", dpi = 300,bg="white")
...
ggsave(filename = "/data/R02/huangtt39/F1_OSN/new_analysis/nobiallelic_gene.pdf",candicate_gene_p, device = "pdf", width = 8, height = 7, units = "cm", dpi = 300,bg="white")


# final RME
RME<-candicate_gene_ratio%>%subset(PM_type=="BI_RME")
RME<-no_biallelic_gene%>%subset(gene%in%RME$gene)
write.xlsx(RME,file="/data/R02/huangtt39/F1_OSN/new_analysis/RME.xlsx")
...
write.xlsx(no_biallelic_ratio,file="/data/R02/huangtt39/F1_OSN/new_analysis/no_biallelic_gene.xlsx")
write.xlsx(no_biallelic,file="/data/R02/huangtt39/F1_OSN/new_analysis/no_biallelic.xlsx")
...
ggsave(filename = "/data/R02/huangtt39/F1_OSN/new_analysis/dist.pdf",dist_p, device = "pdf", width = 11, height = 6, units = "cm", dpi = 300,bg="white")
