...
length(which(table(filtered_mature_OSNs$expressed_OR)>=50)) # 73 groups
...
save(OR_dscore,file="/data/R02/huangtt39/F1_OSN/analysis/merge/OR_annalysis/OR_dscore.RData")
...
write.xlsx(OR_count,file="/data/R02/huangtt39/F1_OSN/analysis/figure/FigureS4/FigureS4a.xlsx")
...
ORgene_FC_p <-ggplot(data=OR_count,aes(x=log2((Paternal_specific+0.1)/(Maternal_specific+0.1))))+
      ...
      coord_flip()
ggsave(filename = "/data/R02/huangtt39/F1_OSN/analysis/figure/FigureS4/OR_cutoff.pdf",ORgene_cellnumber_p+plot_spacer()+ORgene_point_p+ORgene_FC_p+plot_layout(nrow=2,ncol=2,height=c(1,2,1,2),width=c(2,1,2,1)), device = "pdf", width =12, height = 9, units = "cm", dpi = 60,bg="white")
...
for(i in c(10,20,25,30)){
  ...
  stat_poly_eq(aes(label = paste(..eq.label.., ..rr.label.., after_stat(p.value), sep = '~`,`~')),
...
ggsave(filename =  "/data/R02/huangtt39/F1_OSN/analysis/figure/FigureS4/OR_sample_distance_point_seed12345.pdf",sample_distance_point_p+plot_layout(ncol=4,guides = 'collect'), device = "pdf", width =32, height = 8, units = "cm", dpi = 300,bg="white")
colnames(sample_distance)[3] <- "within_median.y"
colnames(sample_distance)[4] <- "between_median.y"
colnames(sample_distance)[5] <- "within_median.x"
colnames(sample_distance)[6] <- "between_median.x"
# 写入文件
write.xlsx(sample_distance, file = "/data/R02/huangtt39/F1_OSN/analysis/figure/FigureS4/FigureS4b.xlsx")
...
mm10_129_PWK_SNP <- read.table("/data/R02/huangtt39/F1_OSN/snp/pwk/pwk_specific.vcf",header=FALSE)[,c(2:6)]%>%rename_with(~c("chr","pos","id","ref","alt"),1:5)
...
txid2gene <- read.table("/data/R02/huangtt39/F1_OSN/mapping/reference/mm10-mask/genes/txid_to_name.txt")
...
write.table(avail_OR_CCDS%>%group_by(gene)%>%dplyr::slice(1),file="/data/R02/huangtt39/F1_OSN/analysis/merge/OR_annalysis/OR_CCDS_txid.txt",col.names = FALSE, row.names = FALSE,quote = FALSE,sep = "\t")

# select the ccds of each OR genes ,and remove the duplicates
avail_OR_CCDS <- unique(avail_OR_CCDS[,c("ccdsId","chrom","cdsStart","cdsEnd","gene","geneId")])
...
unsyn_variant<-read.table("/data/R02/huangtt39/F1_OSN/snp/snp_eff/missense/pwk_specific_missense.txt")[,c(1,4)]
...
write.xlsx(OR_distance,file="/data/R02/huangtt39/F1_OSN/analysis/figure/Figure4/OR_distance_SNP.xlsx")
OR_distance<-read.xlsx("/data/R02/huangtt39/F1_OSN/analysis/figure/Figure4/OR_distance_SNP.xlsx")
...
ggsave(filename = "/data/R02/huangtt39/F1_OSN/analysis/figure/Figure4/OR_difference_ranking.pdf",OR_difference_ranking, device = "pdf", width=16, height = 8, units = "cm", dpi = 300,bg="white")
...
pdf("/data/R02/huangtt39/F1_OSN/analysis/figure/Figure4/OR_distance_heatmap.pdf",height=6.5,width=8.5)
...
# function only for plot
distance_violin_old<-function(OR){
  OR_df <- OR_dscore%>%subset(gene==OR)
  g1_cells <- OR_df%>%subset(merge_dscore_type=="Paternal_specific")
  g1_cells<-g1_cells$barcode
  g2_cells <- OR_df%>%subset(merge_dscore_type=="Maternal_specific")
...
# new function to return data
distance_violin <- function(OR){
  OR_df <- OR_dscore %>% subset(gene == OR)
  
  # 获取细胞
  g1_cells <- OR_df %>% subset(merge_dscore_type == "Paternal_specific") %>% pull(barcode)
  g2_cells <- OR_df %>% subset(merge_dscore_type == "Maternal_specific") %>% pull(barcode)
  ...
  # 保存详细的距离数据
  # Paternal组内距离
  within_g1 <- cosine_distance_df[g1_cells, g1_cells]
  within_g1_df <- data.frame(
    gene = OR,
    cell1 = rep(rownames(within_g1), each = ncol(within_g1)),
    cell2 = rep(colnames(within_g1), times = nrow(within_g1)),
    distance = as.vector(within_g1),
    type = "intra_paternal",
    comparison = "intra",
    stringsAsFactors = FALSE
  )
  within_g1_df <- within_g1_df[within_g1_df$cell1 < within_g1_df$cell2, ]
  
  # Maternal组内距离
  within_g2 <- cosine_distance_df[g2_cells, g2_cells]
  within_g2_df <- data.frame(
    gene = OR,
    cell1 = rep(rownames(within_g2), each = ncol(within_g2)),
    cell2 = rep(colnames(within_g2), times = nrow(within_g2)),
    distance = as.vector(within_g2),
    type = "intra_maternal",
    comparison = "intra",
    stringsAsFactors = FALSE
  )
  within_g2_df <- within_g2_df[within_g2_df$cell1 < within_g2_df$cell2, ]
  
  # 组间距离
  between <- cosine_distance_df[g1_cells, g2_cells]
  between_df <- data.frame(
    gene = OR,
    cell1 = rep(rownames(between), each = ncol(between)),
    cell2 = rep(colnames(between), times = nrow(between)),
    distance = as.vector(between),
    type = "inter",
    comparison = "inter",
    stringsAsFactors = FALSE
  )
  ...
  # 合并所有距离数据
  all_distances <- rbind(within_g1_df, within_g2_df, between_df)
  ...
  within_g2_dis <- cosine_distance_df[g2_cells, g2_cells]
  within_g2_dis <- unlist(lapply(2:nrow(within_g2_dis), function(i){
    within_g2_dis[i, 1:(i-1)]
  }))
  ...
  # 创建绘图数据框
  plot_df <- data.frame(
    distance = c(within_dis, between_dis, perm_dis),
    group = c(rep("intra", length(within_dis)),
              rep("inter", length(between_dis)),
              rep("permutation", length(perm_dis)))
  )
  
  # 统计检验
  # intra vs inter
  df1 <- plot_df %>% filter(group %in% c("intra", "inter"))
  stat1 <- rstatix::wilcox_test(df1, distance ~ group, detailed = TRUE, conf.level = 0.95)
  eff1 <- rstatix::wilcox_effsize(df1, distance ~ group)
  
  # intra vs permutation
  df2 <- plot_df %>% filter(group %in% c("intra", "permutation"))
  stat2 <- rstatix::wilcox_test(df2, distance ~ group, detailed = TRUE, conf.level = 0.95)
  eff2 <- rstatix::wilcox_effsize(df2, distance ~ group)
  
  # inter vs permutation
  df3 <- plot_df %>% filter(group %in% c("inter", "permutation"))
  stat3 <- rstatix::wilcox_test(df3, distance ~ group, detailed = TRUE, conf.level = 0.95)
  eff3 <- rstatix::wilcox_effsize(df3, distance ~ group)
  
  # 合并统计结果
  stats_df <- bind_rows(
    stat1 %>% mutate(comparison = "intra vs inter"),
    stat2 %>% mutate(comparison = "intra vs permutation"),
    stat3 %>% mutate(comparison = "inter vs permutation")
  ) %>%
    select(comparison, group1, group2, n1, n2, statistic, p, conf.low, conf.high)
  
  eff_df <- bind_rows(
    eff1 %>% mutate(comparison = "intra vs inter"),
    eff2 %>% mutate(comparison = "intra vs permutation"),
    eff3 %>% mutate(comparison = "inter vs permutation")
  ) %>%
    select(comparison, effsize, magnitude)
  
  final_stats <- left_join(stats_df, eff_df, by = "comparison") %>%
    mutate(gene = OR) %>%
  ...
  boxplot_p <- ggviolin(plot_df, x = "group", y = "distance", fill = "#766E8E", 
    ...
    stat_compare_means(method = "wilcox.test", 
                       label = "p.format",
                       comparisons = list(c("intra", "inter"),
                                         c("intra", "permutation"),
                                         c("inter", "permutation")),
                       label.y = c(1.9, 2.1, 2.3)) +
    theme_classic() + 
    theme(plot.title = element_text(hjust = 0.5), 
          legend.position = "right", 
          legend.title = element_blank(),
  ...
  # 返回两个结果的列表
  return(list(stats = final_stats, distances = all_distances))
}

pdf("/data/R02/huangtt39/F1_OSN/analysis/figure/Figure4/OR_distance_violin.pdf", height = 2.5, width = 3.5)

OR_genes <- c("Olfr1209", "Olfr1484", "Olfr1423")
all_stats <- list()
all_distances <- data.frame()

for(OR in OR_genes) {
  result <- distance_violin(OR) 
  all_stats[[OR]] <- result$stats
  all_distances <- rbind(all_distances, result$distances)
}

dev.off()

# save data
write.xlsx(do.call(rbind, all_stats), 
          file = "/data/R02/huangtt39/F1_OSN/analysis/figure/Figure4/OR_distance_statistics.xlsx",
          row.names = FALSE)

write.xlsx(all_distances, 
          file = "/data/R02/huangtt39/F1_OSN/analysis/figure/Figure4/OR_example_celldistances.xlsx",
          row.names = FALSE)


myOR_far<-c("Olfr1209","Olfr1484","Olfr1423")
...
# ggsave(filename = "/data/R02/huangtt39/F1_OSN/analysis/figure/Figure4/OR_high_distance_umap.pdf",OR_high_distance_umap1+OR_high_distance_umap2 & NoLegend() & theme(plot.title = element_text(hjust = 0.5,size=8),text=element_text(size=8),axis.text.y=element_text(size=8,color = "black"), axis.text.x=element_text(size=8,  color = "black")), device = "pdf", width =20, height = 10, units = "cm", dpi = 300)
...
# }
# ggsave(filename = "/data/R02/huangtt39/F1_OSN/analysis/figure/Figure6/obj_temp_umap.pdf",umap_list[[1]]+umap_list[[2]]+umap_list[[3]]+umap_list[[4]]+plot_layout(ncol=2) & NoLegend() & theme(plot.title = element_text(hjust = 0.5,size=8),text=element_text(size=8),axis.text.y=element_text(size=8,color = "black"), axis.text.x=element_text(size=8,  color = "black")), device = "pdf", width =16, height = 16, units = "cm", dpi = 300)
...
saveWorkbook(wb, "/data/R02/huangtt39/F1_OSN/analysis/figure/Figure4/OR_differential_gene.xlsx", overwrite = TRUE)


# Figure 4c and Extended Data Fig.4c
...
ggsave(filename = "/data/R02/huangtt39/F1_OSN/analysis/figure/FigureS4c/OR_marker_Volcano_Plot.pdf",marker_p+plot_layout(ncol=4,guides="collect"), device = "pdf", width = 34, height = 8, units = "cm", dpi = 300,bg="white")
...
pdf("/data/R02/huangtt39/F1_OSN/analysis/figure/Figure4/OR_DEG_heat.pdf",height=4.5,width=8)
for(i in 1:3){
  OR_DEG_heat(i)
}
dev.off()
...
saveWorkbook(wb, "/data/R02/huangtt39/F1_OSN/analysis/figure/Figure4/OR_differential_peak.xlsx", overwrite = TRUE)
...
pdf("/data/R02/huangtt39/F1_OSN/analysis/figure/Figure4/OR_DEP_heat.pdf",height=4.5,width=6.5)
for(i in 1:3){
  OR_DEP_heat(i)
}
dev.off()
...
# Figure 4d & Extended Data Fig.4d-f
...
OR_dep_track<-function(i){
  ...
  exp<-ExpressionPlot(obj, features = peak[1,1],group.by="OR_allele_exptype", assay = "RNA")+scale_fill_manual(values=c("#b45a55","#505f84"))+
      theme(text=element_text(size=8))
  file=str_c("/data/R02/huangtt39/F1_OSN/analysis/merge/OR_annalysis/OR_gene_dep/",myOR_far[OR_index],"_",peak[1,1],"_",peak[1,2],".pdf")
...
ggsave(filename = "/data/R02/huangtt39/F1_OSN/analysis/figure/Figure4/unsyn_snp_distance.pdf",SNP_VS_length_p+distance_VS_snp_p+plot_layout(width=c(1,1.5)), device = "pdf", width = 21, height = 8, units = "cm", dpi = 60,bg="white")
...
OR_final_ccds<-read.table(file="/data/R02/huangtt39/F1_OSN/analysis/merge/OR_annalysis/OR_final_ccds.txt",header=TRUE)
...
write.table(OR_GPCR_ccds%>%select(-2),file="/data/R02/huangtt39/F1_OSN/analysis/merge/OR_annalysis/GPCR/OR_GPCR_txid.txt",col.names = FALSE, row.names = FALSE,quote = FALSE,sep = "\t")
...
,file="/data/R02/huangtt39/F1_OSN/analysis/merge/OR_annalysis/OR_classII_CCDS_txid.txt",col.names = FALSE, row.names = FALSE,quote = FALSE,sep = "\t")

write.table(merge(class_OR,avail_OR_CCDS,by.x="gene.name",by.y="gene")%>%subset(gene.name=="Olfr17")%>%group_by(gene.name)%>%dplyr::slice(1)%>%select(gene.name,geneId)
,file="/data/R02/huangtt39/F1_OSN/analysis/merge/OR_annalysis/OR_Olfr17_CCDS_txid.txt",col.names = FALSE, row.names = FALSE,quote = FALSE,sep = "\t")

# Import the constructed class II OR consensus sequence
classII_OR_consense<-read.xlsx("/data/R02/huangtt39/F1_OSN/analysis/figure/Figure5/GPCR/conservation.xlsx",sheet = 1)
...
ggsave(filename = "/data/R02/huangtt39/F1_OSN/analysis/merge/OR_annalysis/GPCR/GPCR_color.pdf",GPRC_p, device = "pdf", width = 30, height = 5, units = "cm", dpi = 300)
...
write.xlsx(classII_OR_consense,file="/data/R02/huangtt39/F1_OSN/analysis/figure/Figure5/GPCR/conservation2color.xlsx",sheet = 1)

distance_conservation<-classII_OR_consense%>%subset(distance%in%c("high","low"))
distance_conservation$distance<-factor(distance_conservation$distance,levels=c("low","high"))
#根据distance 高低画散点图
distance_conservation_p<-ggplot(distance_conservation, aes(x = distance, y = Max.ratio)) +
              geom_boxplot() +  
...
distance_conservation_p<-ggplot(classII_OR_consense%>%subset(distance%in%c("high","low")), aes(x = difference, y = Max.ratio, color = distance)) +
  geom_point(size = 4) +  # 使用geom_point绘制散点
  theme_classic() +  # 使用简洁主题
  scale_color_manual(values=c("#0E8A8F","#F44040"))
ggsave(filename = "/data/R02/huangtt39/F1_OSN/analysis/figure/Figure5/GPCR/distance_conservation.pdf",distance_conservation_p, device = "pdf", width = 10, height = 5, units = "cm", dpi = 300)

# Import the constructed class II OR consensus sequence
classII_OR_consense<-read.xlsx("/data/R02/huangtt39/F1_OSN/analysis/figure/Figure3/GPCR/conservation_add_stress.xlsx",sheet = 1)

ERstress_conservation<-classII_OR_consense%>%subset(Erstress=="Yes")
# 画ER stress conservation 图
ERstress_conservation_p<-ggplot(ERstress_conservation, aes(x = Erstress, y = Max.ratio)) +
              geom_boxplot() +  
              geom_jitter(aes(color = Erstress), width = 0.2, size = 3, alpha = 0.6) +  # 添加散点
              scale_color_manual(values=c("#f5d01f"))+
              scale_y_continuous(limits = c(0.2, 0.8), breaks = c(0.2, 0.4, 0.6,0.8)) +  # 设置Y轴范围和刻
              theme_classic()+ 
              labs(y="Animo acid conservation",x="Top 10 ERstress site")+
              theme(plot.title = element_text(hjust = 0.5), 
                legend.position="right", 
                legend.title = element_blank(),
                text=element_text(size=6))
ggsave(filename = "/data/R02/huangtt39/F1_OSN/analysis/figure/FigureS4/GPCR/ERstress_conservation.pdf",ERstress_conservation_p, device = "pdf", width = 5, height = 5, units = "cm", dpi = 300)
