...
annotation<-import('/data/R02/huangtt39/F1_OSN/mapping/reference/mm10-2020-A-build/gencode.vM23.primary_assembly.annotation.gtf')
...
# Combined_mOSN <- Signac::SetFragments(
#   object = Combined_mOSN,
#   file = c("/data/R02/huangtt39/F1_OSN/mapping/joint202201/mapping_mask/joint202201_mask/outs/atac_fragments.tsv.gz","/data/R02/huangtt39/F1_OSN/mapping/jointF1OSN/mapping_mask/jointF1OSN_mask/outs/atac_fragments.tsv.gz","/data/R02/huangtt39/F1_OSN/mapping/jointF18w/mapping_mask/jointF18w_mask/outs/atac_fragments.tsv.gz")
# )

dirs<-c("/data/R02/huangtt39/F1_OSN/mapping/joint202201/mapping_allele/joint202201_G1count/outs/","/data/R02/huangtt39/F1_OSN/mapping/joint202201/mapping_allele/joint202201_G2count/outs/","/data/R02/huangtt39/F1_OSN/mapping/jointF1OSN/mapping_allele/jointF1OSN_G1count/outs/","/data/R02/huangtt39/F1_OSN/mapping/jointF1OSN/mapping_allele/jointF1OSN_G2count/outs/","/data/R02/huangtt39/F1_OSN/mapping/jointF18w/mapping_allele/jointF18w_G1count/outs/","/data/R02/huangtt39/F1_OSN/mapping/jointF18w/mapping_allele/jointF18w_G2count/outs/") 
...
save(Obj,file="/data/R02/huangtt39/F1_OSN/new_analysis/Obj.RData")
...
high_RME<-no_biallelic_ratio%>%subset(gene%in%high_RME$gene)
write.xlsx(high_RME,file="/data/R02/huangtt39/F1_OSN/new_analysis/high_RME.xlsx")

# Figure 3a and 3b
for(i in c("Ago3","Stk40")){
  file=str_c("/data/R02/huangtt39/F1_OSN/analysis/figure/Figure3/RME/high/",i,".pdf")
...
write.xlsx(final_gene_dscore%>%subset(gene%in%c("Ago3","Stk40"))%>%select(-c(Oripeak, Orirna, rnascore,atacscore,allele_exp,allele_frag,rna_dscore_type)),file="/data/R02/huangtt39/F1_OSN/analysis/figure/Figure3/Figure3ab.xlsx")
...
for(i in top_OR){
  file=str_c("/data/R02/huangtt39/F1_OSN/analysis/figure/Figure2/RME/OR/",i,".pdf")
  ggsave(filename = file,heat_track(i)+reads_bar_gene(i)+plot_layout(nrow=6,ncol=2,heights = c(1,1.2,1.2,1.2,0.6,3),widths=c(4,1)), device = "pdf", width = 20, height =20, units = "cm", dpi = 300,bg="white")
}
...
for(i in c("Ophn1","Diaph2")){
  file=str_c("/data/R02/huangtt39/F1_OSN/analysis/figure/Figure2/RME/chrX/",i,".pdf")
  ggsave(filename = file,heat_track(i)+reads_bar_gene(i)+plot_layout(nrow=6,ncol=2,heights = c(1,1.2,1.2,1.2,0.6,3),widths=c(4,1)), device = "pdf", width = 20, height =20, units = "cm", dpi = 300,bg="white")
}
write.xlsx(final_gene_dscore%>%subset(gene%in%c("Olfr1178","Olfr309","Ophn1","Diaph2"))%>%select(-c(Oripeak, Orirna, rnascore,atacscore,allele_exp,allele_frag,rna_dscore_type)),file="/data/R02/huangtt39/F1_OSN/analysis/figure/FigureS3/FigureS3a-d.xlsx")


# 与NPC gene之间的交集
NPC_RME<-read.xlsx("/data/R02/huangtt39/data/mouse/ESC_NPC_RME.xlsx",sheet = 2,startRow=4)
RME_sameNPC<-RME%>%subset(gene%in%NPC_RME$Gene.Name) #1 genes
...
RME_sameDRN<-RME%>%subset(gene%in%DRN_RME$external_gene_id) # 1 genes
...
RME_sameARN<-RME%>%subset(gene%in%ARN_RME$external_gene_id) #2 genes

# Fish result
Peg3<-data.frame(allele_exp_type=c("P","P","P","M","M","M","B","B","B"),rep=c("rep1","rep2","rep3","rep1","rep2","rep3","rep1","rep2","rep3"),
                ratio=c(90,92,88.2,10,0,0,0,8,11.8),
                method=c(rep("RNA_Fish",9)))
Peg3$method<-factor(Peg3$method,levels=c("RNA_Fish"))
...
ggsave(filename = "/data/R02/huangtt39/F1_OSN/analysis/Fish/experimental/Peg3_Fishresult.pdf",Peg3_p, device = "pdf", width = 4.5, height = 2, units = "cm", dpi = 60,bg="white")


Adcy3<-data.frame(allele_exp_type=c("P","P","P","M","M","M","B","B","B"),rep=c("rep1","rep2","rep3","rep1","rep2","rep3","rep1","rep2","rep3"),
                ratio=c(0,0,0,0,0,2,100,100,98),
                method=c(rep("RNA_Fish",9)))
Adcy3$method<-factor(Adcy3$method,levels=c("RNA_Fish"))
...
ggsave(filename = "/data/R02/huangtt39/F1_OSN/analysis/Fish/experimental/Adcy3_Fishresult.pdf",Adcy3_p, device = "pdf", width = 4.5, height = 2, units = "cm", dpi = 60,bg="white")

Ago3<-data.frame(allele_exp_type=c("P","P","P","M","M","M","B","B","B"),rep=c("rep1","rep2","rep3","rep1","rep2","rep3","rep1","rep2","rep3"),
                ratio=c(48.5,53.9,44.6,30.3,26.9,30.0,21.2,19.2,25.4),
                method=c(rep("RNA_Fish",9)))
Ago3$method<-factor(Ago3$method,levels=c("RNA_Fish"))
Ago3_p<-ggplot(Ago3,aes(x=method,y=ratio,fill=allele_exp_type,color=allele_exp_type))+
    geom_bar(color="black",stat="summary",fun=mean,position="dodge",size=0)+
    geom_point(position = position_jitterdodge(jitter.width = 0.2, dodge.width = 0.9),
             size = 1, shape = 21, stroke = 0.2, color="black") +
    stat_summary(fun.data='mean_sd',geom="errorbar",linewidth=0.15,size=1,width = 0.5,position = position_dodge(0.9))+
...
ggsave(filename = "/data/R02/huangtt39/F1_OSN/analysis/Fish/experimental/Ago3_Fishresult.pdf",Ago3_p, device = "pdf", width = 4.5, height = 2, units = "cm", dpi = 60,bg="white")

Kcnh7<-data.frame(allele_exp_type=c("P","P","P","M","M","M","B","B","B"),rep=c("rep1","rep2","rep3","rep1","rep2","rep3","rep1","rep2","rep3"),
                ratio=c(12,19.3,17.9,84,71.6,76.2,4,9,6),
                method=c(rep("RNA_Fish",9)))
Kcnh7$method<-factor(Kcnh7$method,levels=c("RNA_Fish"))
Kcnh7_p<-ggplot(Kcnh7,aes(x=method,y=ratio,fill=allele_exp_type,color=allele_exp_type))+
    geom_bar(color="black",stat="summary",fun=mean,position="dodge",size=0)+
    geom_point(position = position_jitterdodge(jitter.width = 0.2, dodge.width = 0.9),
             size = 1, shape = 21, stroke = 0.2, color="black") +
    stat_summary(fun.data='mean_sd',geom="errorbar",linewidth=0.15,size=1,width = 0.5,position = position_dodge(0.9))+
...
ggsave(filename = "/data/R02/huangtt39/F1_OSN/analysis/Fish/experimental/Kcnh7_Fishresult.pdf",Kcnh7_p, device = "pdf", width = 4.5, height = 2, units = "cm", dpi = 60,bg="white")
...
ggsave(filename = "/data/R02/huangtt39/F1_OSN/analysis/figure/Figure2/high_RME_rank.pdf",high_RME_count_p+high_RME_ratio_p+plot_layout(heights=c(1,1.5)), device = "pdf", width = 24, height = 14, units = "cm", dpi = 300,bg="white")
...
write.xlsx(no_biallelic_ratio%>%subset(type%in%c("chrX","OR")),file="/data/R02/huangtt39/F1_OSN/analysis/figure/FigureS3/FigureS3e-f.xlsx")
...
ggsave(filename = "/data/R02/huangtt39/F1_OSN/analysis/figure/Figure2/OR_rank.pdf",OR_count_p+OR_ratio_p+plot_layout(heights=c(0.5,1.5)), device = "pdf", width = 16, height = 10, units = "cm", dpi = 300,bg="white")
ggsave(filename = "/data/R02/huangtt39/F1_OSN/analysis/figure/Figure2/chrX_rank.pdf",chrX_count_p+chrX_ratio_p+plot_layout(heights=c(0.5,1.5)), device = "pdf", width = 21, height = 10, units = "cm", dpi = 300,bg="white")
...
library(GOplot)
## 上调基因富集
GOenrich<-function(gene){
  # result<-list()
  entrezid = mapIds(x = org.Mm.eg.db,
                       keys = gene,
                       keytype = "ALIAS",
                       column = "ENTREZID")  
  GO= enrichGO(gene =entrezid,
                 ...
                 pvalueCutoff = 0.05,
                 qvalueCutoff = 0.05)
  GO_symbol <- setReadable(GO, 
                         OrgDb = org.Mm.eg.db, 
                         keyType = "ENTREZID")
  return(as.data.frame(GO_symbol))
}
RME_GOenrichment_result<-GOenrich(RME$gene)
RME_GOenrichment_result$Description<-factor(RME_GOenrichment_result$Description,levels=rev(RME_GOenrichment_result$Description))
write.xlsx(RME_GOenrichment_result,file="/data/R02/huangtt39/F1_OSN/new_analysis/RME_GOenrichment_result.xlsx")
